# 设计要点卡 · 代码地图 · 需求与线上问题推演

> 用途：想在 30 分钟内掌握代码全貌与关键取舍时看这一份。
> - **第一部分**：代码地图（"这个逻辑在哪"，30 秒内能打开）
> - **第二部分**：常见追问与答法（任务书第 29 节列出的问题）
> - **第三部分**：**需求与线上问题推演**（任务书第 30 节：可能给新需求或模拟线上问题）
...
| 层 | 文件 | 数量 |
|---|---|---|
| 单元测试 | `state-machine` / `followup-rules` / `rules-and-handoff` / `batch-window` / `customer-queue` | 75 项 |
| Node 端到端 | `scripts/smoke.mjs` | 56 项 |
| 真实浏览器 | `scripts/e2e-browser.mjs` | 32 项 |

> 三层合计 **163 项断言**。密集回归阶段一条命令跑完：`pnpm test` → `node scripts/smoke.mjs <url> --deepseek --seed-token=<口令>` → `node scripts/e2e-browser.mjs <url> --seed-token=<口令>`。
...
3. **`AiSuggestion` 存了原始请求/响应/token/成本**：这是一张**审计表**。"系统最脆弱的地方在哪""接口报错怎么定位"这类问题，答案都在这张表上 —— 任何一次异常判断都能回放到具体调用
...
5. **成本单价靠环境变量配置**（`DEEPSEEK_PRICE_IN_PER_MTOK` / `_OUT_`）：已按官方价给出默认值，估算取保守上界；官方调价时需要人工同步这个数字
...
另外 `src/lib/agent/followup.ts` 的 `evaluateFollowUp` 也让我满意：9 条规则的判定矩阵写成纯函数后，任务书里的"什么情况跟进/不跟进、成交怎么办、转人工怎么办、是否限制次数"变成了一张可以逐条解释、也能单测的表。
...
第二处是 `pipeline.ts` 的 `runAgent()`：它现在有 400 多行，做了载入、判断、守护、交接、落库五件事。当时为了"一条主干、便于逐行解释"而刻意保持单文件；如果要长期维护，应该拆成 `loadContext / decide / guard / persist` 四个可独立测试的步骤。

### Q10：AI 帮你做了什么，又做错了什么？

见 `AI_CODING_NOTES.md`（写了 9 类真实错误与发现方式）。**一句话总结**：AI 在"写实现"上非常强，但在"这个实现放到真实环境会不会崩""这套规则是否自洽"上仍需人把关 —— 而**能自动跑验证的断言是把关的最有效手段**。24H 内出现的 4 个客户端问题全部逃过了 Node 断言、只在真实浏览器暴露；上线后 3 个用户反馈（重置数据打断会话与链接、种子客户没有判断、判断要刷新好几次才出来）同样逃过了（当时的）37 项接口断言。这两件事直接改变了我对"测试跑过了"的理解。

---

## 第三部分 · 需求与线上问题推演

> 任务书第 30 节：可能给一个新需求或一个模拟线上问题，需要**基于已提交的代码**分析。
> 下面是八个典型场景：两个新需求、三个模拟线上问题，以及三个真实反馈（线上两例、产品一例）。每个场景按"定位 → 改法 → 验证 → 设计上值得强调的点"组织。

### 场景 1（新需求）· 给乐蒙加一条规则："客户提到孩子年龄后，必须推荐对应班型"

**这个问题的关键点**：「企业规则」应该写在哪、怎么让它真正生效。
...
### 场景 2（新需求）· Customer State 增加一个字段："意向等级（高/中/低）"

**这个问题的关键点**：从 AI 输出到数据库再到界面的完整链路。
...
### 场景 3（模拟线上问题）· "同一个客户出现了两条 AI 建议，怀疑 AI 被重复调用"

**这个问题的关键点**：靠**已落库的审计数据**定位，而不是靠猜。
...
### 场景 4（模拟线上问题）· "有客户被重复跟进，销售收到了两次跟进提醒"

**这个问题的关键点**：**幂等键是什么、边界在哪**。
...
### 场景 5（模拟线上问题）· "客户反馈：AI 报了不该报的价"（规则守护失败）
...
### 场景 6（**真实线上反馈**）· "重新部署之后，客户详情页打不开，得重新登录才行"

这是**真实发生过**的问题，也是最有代表性的一例：因为它是"接口断言全绿、用户却用不了"的典型。
...
### 场景 7（**真实线上反馈**）· "客户有聊天记录，但 AI 判断区一直空着，要刷新好几次才出来"
...
> 这会让人直接怀疑整个合并逻辑。现在两种文案是**互斥**的，也有断言守着。"

### 场景 8（**真实产品反馈**）· "客户连发几条之后，销售要干等 8 秒，有点拖节奏"

**现象**：客户连续发几条消息后，销售盯着"约 8 秒后统一判断"干等 —— 尤其在演示时非常明显。

**定位路径（先分清"谁在等"）**：
1. 先问一句：**这 8 秒谁感知得到？** 答案是销售，不是客户 —— 因为当前架构里没有任何东西会自动发给客户
   （AI 是副驾，发送由销售点）。所以这是**销售侧的等待**，不是客户侧延迟。
2. 再问：8 秒是谁加的？模型日志显示一次调用 100–350ms → **延迟几乎全是聚合窗口**。
3. 那窗口能不能短？不能一刀切：窗口的职责是"客户还在打字就别抢答"。
   于是把目标拆成两个：**响应感**（客户在等 → 快）与**判断完整度**（客户还在打字 → 等）。
4. 判据随之从"过了多久"换成"**客户说完了没有**"：问句 2s / 连发补充 3s / 单条陈述 8s。
5. 改完再测：端到端 10.2s → 4.6s，连发合并能力不变；但**还剩 ~2 秒说不清**，于是加分段耗时日志
   → 意外发现**我们自己的耗时埋点是错的**（见下）。

**改法**：
- 新增纯函数 `resolveBatchWindow`（`batch-window.ts`）：三档窗口 + 11 条单测；`fast == base` 即精确退回旧行为
- 窗口值与原因**回传给界面**，销售看到真实秒数（"客户这句在等答复 —— 约 2 秒"），不再写死"约 8 秒"
- 兜底扫描仍按**最长档**判定：它只负责捞回没人管的批次，绝不能抢在正常窗口前面动手
- 顺手修掉耗时埋点：`latencyMs` 从"首字节时间"改成"响应体读完的总耗时"，日志同时输出两者

**设计上值得强调（这一段最能体现工程判断力）**：
> "我一开始也以为 8 秒是必要的。真正让我改主意的是把延迟拆开看：模型只要 100–350ms，
> 那 8 秒 100% 是我自己加的。**判断慢不等于模型慢。**
> 更值钱的是后面那次意外：我加分段日志想找剩下的 2 秒，结果发现『模型段 2315ms』和模型自己日志的『220ms』差了 10 倍 ——
> 同一件事两个数字，必有一个是错的。查下去发现错的是我们：`fetch()` 在响应头到达时就返回了，
> 我在那之后计时，记到的是**首字节时间**。直接打接口验证，真实耗时 2.6–4.3 秒。
> 这件事的意义不只是修一个数字 —— `latencyMs` 是我这套『AI 决策可回放』审计的一部分，
> 它错了会让我在排查慢的时候去怀疑错误的地方。"
