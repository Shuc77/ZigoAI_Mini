...
| 层 | 文件 | 数量 |
|---|---|---|
| 单元测试 | `tests/state-machine.test.ts`、`followup-rules.test.ts`、`rules-and-handoff.test.ts`、`batch-window.test.ts` | 50 项 |
| Node 端到端 | `scripts/smoke.mjs` | 56 项 |
| 真实浏览器 | `scripts/e2e-browser.mjs` | 23 项 |

> 三层合计 **129 项断言**。密集回归阶段一条命令跑完：`pnpm test` → `node scripts/smoke.mjs <url> --deepseek --seed-token=<口令>` → `node scripts/e2e-browser.mjs <url> --seed-token=<口令>`。
...
### 演练 8（**真实产品反馈**）· "客户连发几条之后，销售要干等 8 秒，有点拖节奏"
